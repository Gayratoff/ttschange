...
API_TOKEN = '6550422112:AAFiE_ot0-A0pINDz2CrhV1cwI_Kmf6Qu8o'
...
Menga biror bir matn yozib yuboring

Men sizga o'qib beraman 🤫</b>""")
...
async def send_welcome(message: types.Message):
    ...
    await message.answer(f"""<b>📣 Ovozni o'zgartirish</b>""",reply_markup=types.InlineKeyboardMarkup(row_width=2).add(
            types.InlineKeyboardButton("Erkak kishi", callback_data=f"male"),types.InlineKeyboardButton("Ayol kishi", callback_data=f"women")))
    ...
    try:
        ...
        if mn == 'save':
            with open('{}.ogg'.format(message.message_id), "rb") as voice:
                await sen.delete()
                await bot.send_voice(chat_id=message.from_user.id, voice=voice, caption=f"{message.text}\n🎙 @{bot_user.username}")
